...
def holding_analysis(soup, num_info):
    ...
    * [2]: Maturity
    ...
    try:
        holding_tags = soup.find("div", {"class":"holdings-chart-detailed-section"}).find_all('table')[num_info]
...
def price_plot(ticker):
    ...
    fig = plt.figure(figsize = (9.3, 3.5))
    gs = gridspec.GridSpec(1, 1)    
    ax = plt.subplot(gs[0])
    ax.plot(Adj_Close, color='midnightblue', label = 'Adj Price of %s' %ticker)
    ax.plot(Close, color='goldenrod', label = 'Price of %s' %ticker)
...
def make_slide(ticker):
    try:
        prs = Presentation(save_path + '/etf2_modi.pptx')    # open a pptx file
    
        if ticker in Equity + Real_Estate + Sus_Equity:
            layout = prs.slide_layouts[5]    # slide layout
      ...
      # Bottop part of a slide
        ...
        if ticker in Equity + Real_Estate + Sus_Equity:
            # Country
            text_to_text_placeholder(slide, 13, '국가 분포')
            ct_df = holding_analysis(soup3, 5)
            ct_fig = plot_pie_chart2(ct_df)
            ct_fig_path = save_path + '/ct_fig.png'
            ct_fig.savefig(ct_fig_path, bbox_inches='tight')
            chart_to_picture(slide, 14, ct_fig_path)
            # Sector
            text_to_text_placeholder(slide, 11, '업종 분포')
            sc_df = holding_analysis(soup3, 1)
            ...
            # Trading Volume   
            text_to_text_placeholder(slide, 7, '월간 평균 거래량(만 주)')
            ...
            # Equity Sector
            text_to_text_placeholder(slide, 11, '국가 분포')
            sc_df = holding_analysis(soup3, 5)
            sc_fig = plot_pie_chart(sc_df)
            sc_fig_path = save_path + '/sc_fig.png'
            sc_fig.savefig(sc_fig_path, bbox_inches='tight')
            chart_to_picture(slide, 12, sc_fig_path)
        ...
        prs.save(save_path + '/etf2_modi.pptx')
...
# %% Classification
Equity_US = ['SPY', 'QQQ', 'DIA', 'IJH', 'IJR','IWM']
Equity_US_Smartbeta = ['VTV',  'IWD', 'IWF','USMV', 'QUAL','MTUM',
                       'SDY', 'VIG', 'SPYD', 'SDIV', 'WDIV']
Equity_US_Sec = ['VGT', 'XLK', 'IYW', 'XLV', 'VHT', 'XLF', 'VFH',
                 'XLP', 'XLY', 'XLU', 'XLE', 'XLI', 'XLC', 'XLB']
Equity_WD = ['ACWI', 'ACWV']
Equity_DM = ['VEA', 'EFA', 'IEFA', 'VGK', 'EWJ', 
             'EWA', 'EZU', 'HEDJ', 'FEZ', 
             'EWG','EWQ', 'EWP', 'EWN', 'EWI', 'EWL', 'EWU', 'IQLT', 'EUFN']
Equity_EM = ['VWO', 'EEM', 'IEMG', 'ASHR', 'MCHI', 'KWEB', 'EWY', 'EWT', 'INDA',
             'AIA', 'ILF', 'RSX', 'EWW', 'ECH', 'KSA', 'EZA',
             'EPOL', 'TUR']
Equity_Theme = ['FDN','SOXX', 'XT', 'KOMP', 'IGV', 'ITA', 'JETS', 'ITB', 'EXI', 'IBB', 'IHI', 
                'KBE', 'IBUY', 'MILN', 'GNR', 'IGF', 'SIL', 'XME',
                'MOO', 'BOTZ', 'ARKK', 'ARKG', 'ARKW', 'ARKQ', 'PRNT', 'ESPO', 'CIBR', 'HACK',
                'SKYY', 'CLOU', 'IPO', 'PSP', 'YOLO', 'AWAY',
                'MOAT', 'MGK', 'META', 'BUZZ', 'ARKX', 'BUFR']

Equity = Equity_US + Equity_US_Smartbeta + Equity_US_Sec + Equity_WD +\
         Equity_DM + Equity_EM + Equity_Theme

FI_Broad = ['AGG', 'BNDX', 'BYLD']
FI_Govt = ['BIL', 'SHV', 'SHY', 'IEI', 'IEF', 'TLH', 'TLT', 'GOVT']
FI_IG = ['LQD', 'VCIT', 'VCSH', 'SPSB', 'QLTA']
FI_HY = ['JNK', 'HYG', 'GHYG', 'SHYG']
FI_MBS = ['MBB']
FI_Inflation = ['TIP']
FI_EM = ['EMB', 'EMLC', 'CEMB']
...
Comm_Broad = ['PDBC']
Comm_Energy = ['USO', 'UNG']
Comm_Precious = ['GLD', 'IAU', 'SLV', 'PPLT', 'GLTR', 'PALL']
Comm_Agri = ['DBA']
Commodity = Comm_Broad + Comm_Energy + Comm_Precious + Comm_Agri

RE_Broad = ['VNQ', 'VNQI', 'REET']
RE_Industrial = ['INDS']
RE_Data = ['SRVR']
RE_Mortgage = ['REM']
RE_Residential = ['REZ']
...
Alt_HedgeFund = ['QAI']
Alt_AltIncome = ['PFF', 'CWB', 'BIZD', 'PCEF']

Alternative = Alt_HedgeFund + Alt_AltIncome

Currency = ['UUP', 'FXF', 'FXY', 'FXE', 'FXB', 'FXC', 'FXA']

Multi_Asset = ['IYLD', 'AOA', 'AOR', 'AOM', 'AOK']

Sus_Equity = ['VOTE', 'ESGU', 'ICLN', 'PBW', 'PBD', 'TAN', 'FAN', 'PHO', 'LIT', 'LCTU', 'IDRV', 'DRIV', 'REMX']
